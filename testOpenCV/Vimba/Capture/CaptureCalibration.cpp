...
void CaptureCalibration::LoadXML(FileNode& fn) {
    if (!fn.empty()) {
        ...
        if (!patternString.compare("CHESSBOARD"))
            pattern = CHESSBOARD;
        if (!patternString.compare("CIRCLES_GRID"))
            pattern = CIRCLES_GRID;
        if (!patternString.compare("ASYMMETRIC_CIRCLES_GRID"))
...
void CaptureCalibration::Calibrate() {
    ...
    // Format input image.
    if (flipVertical)
        flip(viewScaled, viewScaled, 0);

    boardFound = false;
    switch (pattern) // Find feature points on the input format
    {
    case CHESSBOARD: boardFound = findChessboardCorners(viewScaled, boardSize, pointBuf, CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FAST_CHECK | CV_CALIB_CB_NORMALIZE_IMAGE);
        break;
    case CIRCLES_GRID: boardFound = findCirclesGrid(viewScaled, boardSize, pointBuf);
        break;
    case ASYMMETRIC_CIRCLES_GRID: boardFound = findCirclesGrid(viewScaled, boardSize, pointBuf, CALIB_CB_ASYMMETRIC_GRID);
        break;
    }
...
void CaptureCalibration::CalcBoardCornerPositions(Size boardSize, float squareSize, vector<Point3f>& corners, Pattern patternType /*= Settings::CHESSBOARD*/) {
    ...
    switch (patternType) {
    case CHESSBOARD:
    case CIRCLES_GRID: for (int i = 0; i < boardSize.height; ++i)
            for (int j = 0; j < boardSize.width; ++j)
                corners.push_back(Point3f(float(j * squareSize), float(i * squareSize), 0));
        break;

    case ASYMMETRIC_CIRCLES_GRID: for (int i = 0; i < boardSize.height; i++)
            for (int j = 0; j < boardSize.width; j++)
