...
public:
		...
		vector<float>& reprojErrs, double& totalAvgErr)
	{
		...
		totalAvgErr = Settings::computeReprojectionErrors(objectPoints, imagePoints, rvecs, tvecs, cameraMatrix, distCoeffs, reprojErrs);
	...
	bool runCalibrationAndSave(Settings& s, Size imageSize, Mat&  cameraMatrix, Mat& distCoeffs, vector<vector<Point2f> > imagePoints)
	{
		...
		if (ok)
			Settings::saveCameraParams(s, imageSize, cameraMatrix, distCoeffs, rvecs, tvecs, reprojErrs,
				imagePoints, totalAvgErr);
		return ok;
	}

	int init(Settings& s, const string inputSettingsFile = "default.xml")
	{
		help();
		...
		Size imageSize;
		Settings::Mode mode = s.inputType == Settings::InputType::IMAGE_LIST ? Settings::Mode::CAPTURING : Settings::Mode::DETECTION;
		clock_t prevTimestamp = 0;
		const Scalar RED(0, 0, 255), GREEN(0, 255, 0);
		const char ESC_KEY = 27;

		namedWindow("Image View", WINDOW_FREERATIO);
		...
		for (int i = 0;; ++i)
		{
			...
			if (mode == Settings::Mode::CAPTURING && imagePoints.size() >= (unsigned)s.nrFrames)
			{
				if (s.runCalibrationAndSave(s, imageSize, cameraMatrix, distCoeffs, imagePoints))
					mode = Settings::Mode::CALIBRATED;
				else
					mode = Settings::Mode::DETECTION;
			}
			...
			if (found)
			{
				...
				if (mode == Settings::Mode::CAPTURING && (!s.inputCapture.isOpened() || clock() - prevTimestamp > s.delay*1e-3*CLOCKS_PER_SEC))
				{
			...
			//----------------------------- Output Text ------------------------------------------------
			string msg = (mode == Settings::Mode::CAPTURING) ? "100/100" : mode == Settings::Mode::CALIBRATED ? "Calibrated" : "Press 'g' to start";
			int baseLine = 0;
			Size textSize = getTextSize(msg, 1, 1, 1, &baseLine);
			Point textOrigin(view.cols - 2 * textSize.width - 10, view.rows - 2 * baseLine - 10);

			if (mode == Settings::Mode::CAPTURING)
			{
			...
			putText(view, msg, textOrigin, 1, 1, mode == Settings::Mode::CALIBRATED ? GREEN : RED);

			if (blinkOutput)
				bitwise_not(view, view);

			//------------------------- Video capture  output  undistorted ------------------------------
			if (mode == Settings::Mode::CALIBRATED && s.showUndistorsed)
			{
				Mat temp = view.clone();
			...
			if (key == 'u' && mode == Settings::Mode::CALIBRATED)
				s.showUndistorsed ^= true;

			if (s.inputCapture.isOpened() && key == 'g')
			{
				mode = Settings::Mode::CAPTURING;
				imagePoints.clear();
			}
		}
	...
	int readSettings(const string calibrationFile, Mat *cameraMatrix, Mat *distortion, double *avg_error, Mat *perViewReprojectionErrors, Mat *extrinsicParameters, Mat *imagePoints);
...
int test_camera_capture() {
	...
	namedWindow("GC2450 blur + threshold", cv::WINDOW_FREERATIO);
	namedWindow("GC2450", cv::WINDOW_FREERATIO);
	Mat frame;
	Mat output;
	Mat dest;
	for (;;)
	{
		...
		GaussianBlur(frame, output, Size(3, 3), 1.0);

		//blur(frame, output, Size(21, 21));

		threshold(output, dest, 100, 255, CV_THRESH_BINARY_INV);

		imshow("GC2450", frame);
		imshow("GC2450 blur + threshold", dest);
		//imshow("GC2450 lap", lap);
